...
if ( is_single() ) :
    ...
    if ( $intro_featured ) : // Check if an intro has been selected in posts/portfolios - $intro_featured = intro ID
        ...
        $intro_full_height = get_post_meta( $page_id, '_mokaine_intro_single_full_height', true );
        $intro_scroll_arrow = get_post_meta( $page_id, '_mokaine_intro_single_scroll_arrow', true );
        $intro_darken = get_post_meta( $page_id, '_mokaine_intro_single_darken', true );
    ...
    if ( $intro_meta) : // Check if an intro has been selected in pages - $intro_meta = intro ID
        ...
        // Store metabox values -> Select Intro, Settings
        $intro_full_height = get_post_meta( $page_id, '_mokaine_intro_full_height', true );
        $intro_autoplay = get_post_meta( $page_id, '_mokaine_intro_autoplay', true );
        $intro_navigation = get_post_meta( $page_id, '_mokaine_intro_navigation', true );
        $intro_pagination = get_post_meta( $page_id, '_mokaine_intro_pagination', true );
        $intro_scroll_arrow = get_post_meta( $page_id, '_mokaine_intro_scroll_arrow', true );
        $intro_darken = get_post_meta( $page_id, '_mokaine_intro_darken', true );
        ...
        // Select intro -> Precomposed Intro
            ...
            $slides = get_post_meta( $intro_meta, '_mokaine_single_slide', true ); // true per entrare dentro l'array
                ...
                <div id="intro-wrap" class="<?php if ( $intro_full_height ) echo 'full-height' ?>" data-height="33.333">
                    ...
                    <div id="intro" class="preload <?php if ( $intro_scroll_arrow ) echo 'more-button' ?> <?php if ( $intro_darken ) echo 'darken' ?>" data-autoplay="<?php echo $intro_autoplay ?>" data-navigation="<?php esc_attr_e( $intro_navigation ) ?>" data-pagination="<?php esc_attr_e( $intro_pagination ) ?>" data-transition="fade">
                        ...
                        <?php foreach ( $slides as $slide ) : ?>
                            ...
                            $slide_type = get_meta( $slide, '_mokaine_slide_type' );
                            $bg_color = get_meta( $slide, '_mokaine_slide_bg_color' );
                            $bg_image_id = get_meta( $slide, '_mokaine_slide_select_image_id' );
                            ...
                            $font_color = get_meta( $slide, '_mokaine_slide_font_color' );
                            $slide_title = get_meta( $slide, '_mokaine_slide_title' );
                            $slide_subtitle = get_meta( $slide, '_mokaine_slide_subtitle' );
                            $credits_box = get_meta( $slide, '_mokaine_slide_credits_box' );                    
                            $show_button = get_meta( $slide, '_mokaine_slide_button_show' );
                            $button_text = get_meta( $slide, '_mokaine_slide_button_text' );
                            $button_link = get_meta( $slide, '_mokaine_slide_button_link' );
                            $button_style = get_meta( $slide, '_mokaine_slide_button_style' );                    
                            $button_color = get_meta( $slide, '_mokaine_slide_button_color' );
                            $button_new_tab = get_meta( $slide, '_mokaine_slide_button_new_tab' );
                            $mockup_caption = get_meta( $slide, '_mokaine_slide_mockup_layout');
                            $map_latitude = get_meta( $slide, '_mokaine_map_latitude' );
                            $map_longitude = get_meta( $slide, '_mokaine_map_longitude' );
                            $map_zoom = get_meta( $slide, '_mokaine_map_zoom' );
                            $map_style = get_meta( $slide, '_mokaine_map_style' );
                            $map_marker = get_meta( $slide, '_mokaine_map_marker' );
                            $map_tooltip = get_meta( $slide, '_mokaine_map_tooltip' );
                            $extra_class = get_meta( $slide, '_mokaine_slide_extra_class' );
                            ?>
                            ...
                            $placeholderUrl = get_template_directory_uri() . '/mokaine/includes/img/placeholder.png';
